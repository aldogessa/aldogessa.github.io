...
# -----------------------------
# STESSA FUNZIONE DELLE DESTINAZIONI
# -----------------------------
...
def export_related_data_to_existing_table():
    try:
        # Layer padre
        parent_layer_list = QgsProject.instance().mapLayersByName("_Particelle_")
        if not parent_layer_list:
            iface.messageBar().pushMessage("Errore", "Layer '_Particelle_' non trovato!", level=3)
            return
        parent_layer = parent_layer_list[0]

        selected_parents = parent_layer.selectedFeatures()
        if not selected_parents:
            iface.messageBar().pushMessage("Avviso", "Nessun elemento selezionato nel layer '_Particelle_'.", level=2)
            return
        ...
        # Layer figlio
        child_layer_list = QgsProject.instance().mapLayersByName("Analisi urbanistica")
        if not child_layer_list:
            iface.messageBar().pushMessage("Errore", "Layer 'Analisi urbanistica' non trovato!", level=3)
            return
        child_layer = child_layer_list[0]
        ...
        # -------------------------------------------------
        # COSTRUISCO UNA LISTA PIATTA DI "RIGHE LOGICHE"
        # (come nello script Destinazioni)
        # -------------------------------------------------
        righe = []

        # Mappa VIRTID → parent (per FOGLIO, ALLEGATO, ecc.)
        parents_by_virtid = {p["VIRTID"]: p for p in selected_parents}

        for child in child_layer.getFeatures():
            virtid = child["VIRTID"]
            if virtid not in parents_by_virtid:
                continue  # figlio non collegato a una particella selezionata

            parent = parents_by_virtid[virtid]

            # Filtro ORDINARIO
            if applica_filtro and not str(child["TEMA"]).startswith(codici_ammessi):
                continue

            foglio = int(parent["FOGLIO"])
            allegato = child["ALLEGATO"]
            mappale = child["MAPPALE"]
            ...
            righe.append({
                ...
                "TEMA": child["TEMA"],
                "ZONA": child["ZONA"],
                "DETTAGLIO": child["DETTAGLIO"],
                "NORME": child["NORME"],
                "PERCENT_V": child["PERCENT_V"]
            })

        if not righe:
            iface.messageBar().pushMessage("Avviso", "Nessun dato collegato alle particelle selezionate.", level=2)
            return

        # -------------------------------------------------
        # ORDINAMENTO IDENTICO ALLO SCRIPT DESTINAZIONI
        # -------------------------------------------------
        righe_ordinate = sorted(
            righe,
            key=lambda r: (
                int(r["FOGLIO"]),
                parse_mappale(r["MAPPALE"]),
                str(r["TEMA"]),
                str(r["ZONA"]),
                r["PERCENT_V"]
            )
